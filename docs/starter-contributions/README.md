...
**Origin:** every item here was built and **battle-tested in production** in a fork of that starter —
**Huddle Up** (`huddleup.health`), a cooperative wellness app. Each entry names the originating
DECISION / work-log / retrospective so the starter's own Claude can judge fit and see the incident it
solved. Product-specific work (the wellness domain: health/coach/huddle/nutrition/sleep/Fitbit/sync)
has been deliberately **excluded** — only starter-generic improvements are here.

**How the starter should use this.** Each entry is a self-contained mini-spec: *origin → problem → the
generic fix → where it lands in the single-root starter → implementation steps → verification →
classification*. Point a Claude session at this file and work items top-down, or lift any entry into an
issue. **Path mapping:** this fork is a monorepo, so its `web/src/...` maps to the starter's `src/...`;
`.claude/`, `.github/`, `docs/` map 1:1; `mobile/...` has no starter equivalent (see §G).
...
**Caveat.** The starter has had **zero commits since 2026-05-21** (dormant). These are queued proposals;
they land only if it's being maintained.

**Suggested PR order** (highest leverage / lowest risk first):
1. **Safety guards bundle** (§C1–C4) — the tripwires + hook + QA gates. Pure upside, tiny surface.
2. **`cachedAuth` + `cache()` flags** (§B1) and **timezone-safe `<FormattedDate>`** (§E1) — drop-in wins on files the starter already ships.
3. **DB util pack** (§A1–A4) — `isUniqueViolation`, sql-date tripwire, Neon-http gotchas, token/crypto utils.
4. **e2e prod-DB isolation** (§D1) + **DB-sync action** (§D2) — deployment/test safety.
5. **The reusable features** (§F) and **UI primitives** (§E) — larger, opt-in.
6. **Structural** (§G) — the monorepo + mobile-developer agent + this `downstream-sync` skill.
