 ...
 *
 * Harvested from ~/git/npvitals/scripts/check-agent-symbols.mjs (161 lines,
 * see docs/work-log/2026-09-07-verification-contracts.md Phase 3, Increment
 * 3) after that sibling repo caught five of nine agent files still importing
 * a retired `requireRole` helper eleven days after removal. This repo has no
 * monorepo `apps`/`packages` split, so the search root below is `src` (agent
 * files here reference `src/`, never `apps/`), and the harvest's own
 * `requireRole` entry does not apply — RETIRED ships empty until a real
 * incident (or Increment 3's own audit) creates the first entry.
 *
 * THE RULE
 * --------
 ...
 *
 * Run: `node scripts/check-agent-symbols.mjs` (also `npm run check:agent-symbols`,
 * folded into `npm run check` and `/pre-push` Step 3f).
 */
 ...
 * to find the work-log.
 *
 * Empty on purpose (Phase 3 ruling): the harvest's only entry (`requireRole`)
 * is npvitals-specific and does not exist in this repo's history. An empty
 * list means this tripwire is a structural no-op until a real stale symbol
 * is found — see the work-log's Phase 4 section for how it was proven to
 * actually fire (a temporary non-empty entry, reverted).
 */
export const RETIRED = [];

/** Is this symbol genuinely absent from the source? Keeps the list honest. */
export function isActuallyRetired(symbol) {
  try {
    // Look for a real export, not a mention. Comments and corrective notes in
    // source files must not keep an entry alive.
    const out = execFileSync(
      "grep",
      ["-rEl", `export (async )?(function|const) ${symbol}\\b`, "src"],
...
function main() {
  ...
  console.log(
    `  ${agentFiles.length} agent file(s), ${active.length} retired symbol(s) enforced` +
      (staleEntries.length
        ? `, ${staleEntries.length} list entry(ies) ignored as no longer retired`
        : ""),
  );
...
const isMain =
  ...
  process.argv[1]?.endsWith("check-agent-symbols.mjs");
